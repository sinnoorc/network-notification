  ...
  Widget build(BuildContext context) {
    ...
    if (showNetworkUpdates) {
      _listenToNetworkChanges(bottomInternetNotificationPadding,
          messenger: networkStateMessenger, connectivityCheckAddress: connectivityCheckAddress);
    }
    return SizedBox(
      key: _tKey,
      child: child,
...
OverlayState? get _overlayState {
  ...
  assert(navigator != null, '''Cannot find OTS above the widget tree, unable to show overlay''');
    ...
    switch (event) {
      case ConnectivityResult.wifi:
        _checkInternetConnectionAndShowStatus(bottomPadding,
            connectivityCheckAddress: connectivityCheckAddress, messenger: messenger);
        break;
...
Future<void> _showNetworkStateWidget(NetworkState state, NetworkStateMessenger? messenger, double bottomPadding) async {
  try {
    ...
    _printError('''Caught an exception while trying to show NetworkStatusWidget''');
    throw err;
  }
}
...
Future<void> _hideNetworkStateWidget() async {
  try {
    ...
    _printError('''Caught an exception while trying to hide NetworkStatusWidget''');
    throw err;
  }
}
...
Future<void> showLoader({bool isModal = false, Color? modalColor, bool modalDismissible = true}) async {
  try {
    ...
    final _child = Center(
      child: _loadingIndicator ?? (Platform.isAndroid ? CircularProgressIndicator() : CupertinoActivityIndicator()),
    );
    await _showOverlay(
      child: isModal
          ? Stack(
    ...
    debugPrint('''Caught an exception while trying to show a Loader\n${err.toString()}''');
    throw err;
  }
}
...
///----------------------------------Toasts------------------------------------
Future<void> bakeToast(String message, {Color? backgroundColor, TextStyle? textStyle, ToastType type = ToastType.normal}) async {
  try {
    Widget? _toast;
    switch (type) {
...
//    }
    ...
    _printError('''Caught an exception while trying to insert Overlay\n${err.toString()}''');
    throw err;
  }
}
...
Future<void> _hideOverlay(_OverlayType type) async {
  try {
    ...
    _printError('''Caught an exception while trying to remove Overlay\n${err.toString()}''');
    throw err;
  }
}
